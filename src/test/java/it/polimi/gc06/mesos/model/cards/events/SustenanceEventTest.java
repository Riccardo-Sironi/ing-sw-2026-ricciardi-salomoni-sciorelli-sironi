...
public class SustenanceEventTest {
    ...
    void testSettersAndGetters() {
        SustenanceEvent event = new SustenanceEvent();
        assertEquals(-1, event.getPrestigeLoss());

        event.setPrestigeLoss(3);
        assertEquals(3, event.getPrestigeLoss());
    }
    ...
    // HELPER: Crea un mazzo fasullo con il numero totale di carte specificato,
    // garantendo che il sum() delle liste dia il valore corretto per attivare i branch if (requiredFood > 0)
    private EnumMap<CharacterType, ArrayList<CharacterCard>> createDeckWithTotalCards(int totalCards) {
        ...
        deck.put(CharacterType.HUNTER, cards);
        return deck;
    }

    @Test
    void testResolveEvent_0RequiredFood_NoTokensChanged() {
        when(player.getCharacterDeck()).thenReturn(createDeckWithTotalCards(0));
    ...
    void testResolveEvent_RequiredFoodMatchesGathererDiscount_NoTokensChanged() {
        // 3 characters, 1 gatherer (provides default discount of 3)
        when(player.getCharacterDeck()).thenReturn(createDeckWithTotalCards(3));
    ...
    void testResolveEvent_HasEnoughFood_RemovesFoodNoPrestigeLoss() {
        // 4 character cards, 0 gatherers -> required food = 4
        when(player.getCharacterDeck()).thenReturn(createDeckWithTotalCards(4));
    ...
    void testResolveEvent_NotEnoughFood_RemovesAllFoodAndLosesPrestige() {
        // 5 character cards, 0 gatherers -> required food = 5
        when(player.getCharacterDeck()).thenReturn(createDeckWithTotalCards(5));
    ...
    void testResolveEvent_NoFoodAtAll_LosesOnlyPrestige() {
        // 2 character cards, required food = 2
        when(player.getCharacterDeck()).thenReturn(createDeckWithTotalCards(2));
        when(player.getFoodTokens()).thenReturn(0);
    ...
    void testResolveEvent_WithGathererBonusDiscountBuilding() {
        // 4 character cards, 1 gatherer -> base required = 4 - 3 = 1
    ...
    void testResolveEvent_WithArtistDiscountBuilding() {
        // 4 character cards, 0 gatherers -> base required = 4
    ...
    void testResolveEvent_WithInventorDiscountBuilding() {
        // 5 character cards, 0 gatherers -> base required = 5
    ...
    void testResolveEvent_WithMultipleDiscountBuildings_OverDiscount() {
        // 5 character cards, 1 gatherer (default 3), 1 artist, 1 inventor
    ...
    void testEqualsAndHashCode() {
        SustenanceEvent e1 = new SustenanceEvent(Era.ERA_I, 2);
        SustenanceEvent e2 = new SustenanceEvent(Era.ERA_I, 2);
        SustenanceEvent e3 = new SustenanceEvent(Era.ERA_I, 3);
